...
- **Command Caching**: Added static caching to `ProcessRunner::commandExists` to eliminate redundant shell execution checks (~1-5ms savings per call)
- **Parsing Optimization**: Replaced regex splitting with `explode` in `LinuxProcStatParser` and `LinuxProcPidStatParser` for reduced CPU overhead

#### Documentation

- **Link Fixes**: Fixed internal documentation links to include `.md` extension for correct navigation on Cbox.dk and GitHub
- **Rebranding**: Updated documentation URLs and references from PHPeek to Cbox

**Full Changelog**: https://github.com/cboxdk/system-metrics/compare/v2.0.0...v2.0.1
